...
class MigPackAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        private val PKG_REGEX = Pattern.compile("Paketler\s*\((\d+)\)", Pattern.CASE_INSENSITIVE or Pattern.UNICODE_CASE)
        private val DATE_REGEX = Pattern.compile("\b(\d{2})\s+([A-Za-zÇĞİÖŞÜçğıöşü]+)\s+([A-Za-zÇĞİÖŞÜçğıöşü]+)\b")
